...
const FerronyxFAQ = () => {
    return (
        <section id="faqs" className="relative w-full bg-[#050505] py-32 px-6 border-t border-white/[0.05]">
             {/* Subtle Ambient Light */}
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] bg-white/[0.01] rounded-full blur-[120px] pointer-events-none"></div>
                ...
                <Accordion type="single" collapsible className="space-y-4">
                    {faqs.map((faq, index) => (
                        <AccordionItem
                            key={index}
                            value={`item-${index}`}
                            className="rounded-xl border border-white/[0.08] bg-[#0A0A0B] px-2"
                        >
                            <AccordionTrigger className="px-4 py-6 text-lg font-medium text-[#EDEDED] hover:text-white transition-colors [&[data-state=open]]:text-white text-left">
                                {faq.question}
                            </AccordionTrigger>
                            <AccordionContent className="px-4 pb-6 text-[#8A8F98] text-base leading-relaxed">
                                {faq.answer}
                            </AccordionContent>
                        </AccordionItem>
                    ))}
