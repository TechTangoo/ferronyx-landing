...
const FerronyxFAQ = () => {
    return (
        <section id="faqs" className="relative w-full bg-black py-24 px-6 border-t border-white/5">
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] bg-zinc-800/10 rounded-full blur-[120px] pointer-events-none opacity-40"></div>

            <ScrollReveal className="max-w-3xl mx-auto">
                <div className="text-center mb-16">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm text-xs font-medium text-zinc-400 w-fit mb-6">
                        FAQ
                    </div>
                ...
                <Accordion type="single" collapsible className="space-y-4">
                    {faqs.map((faq, index) => (
                        <AccordionItem
                            key={index}
                            value={`item-${index}`}
                            className="rounded-2xl border border-white/5 bg-zinc-900/10 px-2"
                        >
                            <AccordionTrigger className="px-4 py-6 text-lg font-medium text-zinc-300 hover:text-white transition-colors [&[data-state=open]]:text-white">
                                {faq.question}
                            </AccordionTrigger>
                            <AccordionContent className="px-4 pb-6 text-zinc-500 text-base leading-relaxed">
                                {faq.answer}
                            </AccordionContent>
                        </AccordionItem>
                    ))}
